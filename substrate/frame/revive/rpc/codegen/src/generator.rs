	...
	fn generate_type(&mut self, name: String, schema: &Schema) -> TypePrinter {
		...
		let content = match &schema.contents {
			...
			&SchemaContents::Literal(Literal::Array(ArrayLiteral { items: Some(ref schema) })) => {
				let mut type_info =
					self.type_info(schema).expect("Anonymous array type not supported");
...
impl TypeNameProvider for TypeGenerator {
	fn record_inline_type(&mut self, type_name: String, schema: &Schema) -> TypeInfo {
		self.collect(&type_name, ReferenceOrSchema::Schema(schema.clone()));
		TypeInfo { name: type_name, required: Required::Yes, array: false }
	}
	...
	fn type_info(&mut self, schema: &Schema) -> Option<TypeInfo> {
		match &schema.contents {
			...
			SchemaContents::Literal(Literal::Object(_)) => None,
			v => {
...
mod test {
	...
	#[test]
	fn generate_type_with_inline_variant_works() {
		let specs = read_specs().unwrap();
		let mut generator = TypeGenerator::new();
		let res = generator.generate_type(
			"SyncingStatus".to_string(),
			specs.get_schema("#/components/schemas/SyncingStatus").unwrap(),
		);
		let mut buffer = String::new();
		res.print(&mut buffer);
